...
# It runs during the post-appdeploy hook phase.

set -x # Enable verbose debugging
LOG_FILE="/tmp/nginx_setup_debug.log"

echo "Running post-appdeploy Nginx configuration script. Output redirected to $LOG_FILE" >> "$LOG_FILE" 2>&1
echo "Starting Nginx config creation and activation..." >> "$LOG_FILE" 2>&1

NGINX_TARGET_DIR="/etc/nginx/conf.d"
FINAL_CONFIG_FILE="$NGINX_TARGET_DIR/elasticbeanstalk-nginx-http-https.conf"

echo "Checking if Nginx config directory $NGINX_TARGET_DIR exists..." >> "$LOG_FILE" 2>&1
if [ ! -d "$NGINX_TARGET_DIR" ]; then
  echo "Directory $NGINX_TARGET_DIR does not exist. Creating it..." >> "$LOG_FILE" 2>&1
  sudo mkdir -p "$NGINX_TARGET_DIR" >> "$LOG_FILE" 2>&1 || { echo "Failed to create directory $NGINX_TARGET_DIR!" >> "$LOG_FILE" 2>&1; exit 1; }
  echo "Directory $NGINX_TARGET_DIR created." >> "$LOG_FILE" 2>&1
else
  echo "Directory $NGINX_TARGET_DIR already exists." >> "$LOG_FILE" 2>&1
fi
ls -ld "$NGINX_TARGET_DIR" >> "$LOG_FILE" 2>&1 # Show permissions of the target directory

echo "Attempting to write Nginx config directly to $FINAL_CONFIG_FILE..." >> "$LOG_FILE" 2>&1
...
server {
  ...
  ssl_certificate /etc/letsencrypt/live/[forum-backend-env.eba-rkkugpwy.ap-southeast-2.elasticbeanstalk.com/fullchain.pem](https://forum-backend-env.eba-rkkugpwy.ap-southeast-2.elasticbeanstalk.com/fullchain.pem);
  ssl_certificate_key /etc/letsencrypt/live/[forum-backend-env.eba-rkkugpwy.ap-southeast-2.elasticbeanstalk.com/privkey.pem](https://forum-backend-env.eba-rkkugpwy.ap-southeast-2.elasticbeanstalk.com/privkey.pem);
  ...
  location / {
    ...
    proxy_pass [http://127.0.0.1:5001](http://127.0.0.1:5001); # Proxy to your Node.js app
