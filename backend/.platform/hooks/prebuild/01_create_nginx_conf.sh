...
echo "Attempting to create Nginx configuration file using sudo sh -c..."

# Define the target file path
NGINX_CONFIG_FILE="/etc/nginx/conf.d/elasticbeanstalk-nginx-http-https.conf"

# Use 'sudo sh -c' to execute the entire cat <<EOF block as root.
# This provides a more robust execution context for file creation.
sudo sh -c "cat <<'EOF_INNER' > $NGINX_CONFIG_FILE
